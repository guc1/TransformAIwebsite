...
## 2025-09-15 — "Blog editor gate rejects correct code"

- **User ask / Bug:** Entering `cake2025` returned "Invalid code" and access was denied.
- **Root cause:** Middleware guarded `/api/admin/blog/login`, so the session cookie was never set.
- **Fix:**
  - Allowed the login route to bypass middleware protection.
- **Files:** `apps/www/middleware.ts`.
