...
const DAILY_GROWTH_FACTOR = 1.01;
...
function calculateHoursSaved(nowMs: number): number {
  ...
  const partialAddition = Math.min(currentDayAddition, currentDayAddition * currentDayProgress);
  ...
  useEffect(() => {
    const update = () => {
      setHoursSaved((previous) => {
        const next = Math.floor(calculateHoursSaved(Date.now()));
