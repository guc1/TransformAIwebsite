...
function ProjectShowcase({
  ...
  const getProjectCopy = (suffix: string) => {
    if (!contentKey) {
      return undefined;
    }

    try {
      return projectCopy(`${contentKey}.${suffix}`);
    } catch (error) {
      return undefined;
    }
  };

  const title = getProjectCopy("title");
  const description = getProjectCopy("description");
  const result = getProjectCopy("result");
  const cta = getProjectCopy("cta");
  const field = getProjectCopy("field");
  const name = getProjectCopy("name");
...
function FrameworkSwitcher({
  ...
  const getLabel = (framework: Framework) => {
    if (!framework.contentKey) {
      return framework.name;
    }

    try {
      return projectCopy(`${framework.contentKey}.label`);
    } catch (error) {
      return framework.name;
    }
  };
