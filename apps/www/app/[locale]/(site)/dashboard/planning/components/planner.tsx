      ...
      <div className="rounded-3xl border border-white/15 bg-white/[0.04] p-6 shadow-[0_30px_120px_rgba(15,23,42,0.45)]">
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
          {days.map((day) => {
            ...
            const blocked = day.slots.filter(
              (slot) => slot.status === "unavailable" && slot.isPublished,
            ).length;
            ...
            return (
              <button
                ...
                aria-label={`${weekday} ${dayNumber} ${month}. ${statusLabel}. ${t("calendar.count.available", { count: available })}, ${t("calendar.count.booked", { count: booked })}, ${t("calendar.count.blocked", { count: blocked })}, ${t("calendar.count.hidden", { count: hidden })}`}
              >
                ...
                <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[11px] leading-snug text-white/65">
                  <span>{t("calendar.metrics.available", { count: available })}</span>
                  <span className="text-right">{t("calendar.metrics.booked", { count: booked })}</span>
                  <span>{t("calendar.metrics.blocked", { count: blocked })}</span>
                  <span className="text-right text-white/60">
                    {t("calendar.metrics.hidden", { count: hidden })}
                  </span>
                  <span className="col-span-2 text-right text-white/55">
                    {t("calendar.metrics.total", { count: total })}
                  </span>
                </div>
              </button>
            );
        ...
        <div className="mt-8 space-y-2 text-xs text-white/60">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <LegendBadge color="bg-slate-400/35" label={t("legend.blocked")} />
