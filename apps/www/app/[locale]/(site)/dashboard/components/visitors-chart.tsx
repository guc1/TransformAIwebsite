...
export function VisitorsChartSection({ hourly, daily, tabLabels, emptyMessages }: VisitorsChartSectionProps) {
  ...
  const chartMinWidth = Math.max(data.length * minBarWidth, 960);
                  ...
                  <Tooltip key={`${activeTab}-${bucket.id}`}>
                    <TooltipTrigger asChild>
                      ...
                      >
                        <div
                          className={cn(
                            "flex h-52 w-full items-end overflow-hidden rounded-xl border border-white/10 bg-white/5",
                            !showBar && "border-transparent bg-transparent",
                          )}
                        >
                          <div
